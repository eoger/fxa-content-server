...
function (chai, sinon, p, Constants, Assertion, ProfileClient,
  ...
  describe('models/account', function () {
    ...
    describe('destroyOAuthToken', function () {
      it('destroys an OAuth token', function () {
        sinon.stub(oAuthClient, 'destroyToken', function () {
          return p();
        });

        return account.destroyOAuthToken('token')
          .then(function () {
            assert.isTrue(oAuthClient.destroyToken.calledWith('token'));
          });
      });
    });
  });
